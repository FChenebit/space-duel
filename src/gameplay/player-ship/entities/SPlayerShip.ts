...
export class SPlayerShip implements ISMovingObject {
  ...
  constructor(initialX: number, initialY: number, newRepresentationId: string, initialRotation: number,
      newSpeed : number,newMaxSteering: number,newAcceleration: number, newMaxSpeed: number, newLaserWeapon: SWeapon, newMissileWeapon: SWeapon, newMineWeapon: SWeapon) {
    this.x = initialX;
    this.y = initialY;
    this.rotation = initialRotation;
    this.steering = 0;
    this.maxSteering = newMaxSteering;
